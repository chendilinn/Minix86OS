...
void init_cursor()	//接着loader显示的信息显示
{
	outb(0x3d4, 14);
	uint8_t cursor_h = inb(0x3d5);//光标高八位
	outb(0x3D4, 15);
	uint8_t cursor_l = inb(0x3d5);//光标低八位
	uint16_t cursor_pos = (cursor_h << 8) | cursor_l;
	cursor_y = cursor_pos/80;
	cursor_x = cursor_pos%80;
}
