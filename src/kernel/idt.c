...
#define IDT_DESC_CNT 255	 // 目前总共支持的中断数
...
static gate_desc idt[IDT_DESC_CNT];   // idt是中断描述符表,本质上就是个中断门描述符数组
...
static void idt_desc_init(void)
{
   ...
   for (i = 0; i < IDT_DESC_CNT; i++) {
      ...
      //printk("intr_entry_table[%d]=%x\n",i,intr_entry_table[i]);
   }
...
void general_intr_handler(uint8_t vec_nr) {
   ...
   printk("int vector: 0x%x %d %s\n", vec_nr, timer_cnt++, intr_name[vec_nr]);
}
